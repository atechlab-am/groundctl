...
def _create_env(client, operator_token, cv, name="dev", path_name="main", position=0, publish_prefix="dev"):
    ...
    # path-order gate). This helper chains the new environment after
    # Library then assigns+promotes the content view's already-published
    # version 1 straight to it, so callers keep getting back a fully
    # linked, published environment exactly like before. No caller in this
    # file uses position>0, so further chaining isn't needed here.
