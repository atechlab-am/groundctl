...
**No path enforcement**: a content view can be assigned+promoted to any environment directly, in any order — position in the path is organizational only, not a gate. You don't need to promote `jammy-baseline` to Library before assigning it to `jammy-qa`, and nothing requires the environments before it in the path to already have that content view live. Create a third environment (e.g. `"name": "jammy-dev"`, no prior — appends after `jammy-qa`) and assign content views to it in whatever order fits your workflow. To insert an environment somewhere other than the end, pass `"prior_environment_id"` explicitly — everything currently after that point shifts back by one position to make room.
