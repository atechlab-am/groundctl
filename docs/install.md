...
This does, in order: `git fetch`/`checkout` the checkout it was installed from to the latest `main` (the released/stable branch — see `docs/releasing.md`), re-renders and re-checks the `groundctl`/`groundctl-worker`/`groundctl-beat` systemd units (always — see below), then, whenever `main` actually moved (whether or not `VERSION` itself changed — several ordinary commits can land on `main` between version bumps; gating a redeploy on the version string alone previously let a checkout silently skip syncing/restarting even though it had genuinely pulled newer app code, see `CHANGELOG.md`), rebuilds the web UI, resyncs app code, updates Python dependencies, applies pending database migrations, and restarts all three services. Running it again with nothing new to pull still reports "already up to date" for the code/migrations, but has already repaired anything.

The systemd-unit check runs unconditionally, independent of whether any commit changed — cheap (render + `cmp` against what's installed), and it means a unit that went stale for a reason *other* than app code changing (e.g. it was written by an older version of `install.sh`/`groundctl-maintain` before some config it depends on existed) gets repaired the next time `upgrade` runs, not just on a full `install.sh` re-run.
